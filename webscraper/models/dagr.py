...
class Dagr:
    ...
    def load_configuration(self):
        ...
        my_conf.read([expanduser("~/.config/dagr/dagr_settings.ini"), path_join(getcwd(), "dagr_settings.ini")])
    ...
    # get a page's html or the contents of a file
    def get(self, url, file_name=None):

        if (file_name and not self.overwrite and path_exists(self.directory + file_name)):
        ...
        if self.browser.response.status_code != req_codes.ok:
            raise DagrException("incorrect status code: " + str(self.browser.response.status_code))
    ...
    def deviant_get(self, mode):
        ...
        if not pages:
            ...
            try:
                CU.MakeDir(self.directory + self.deviant + "/" + mode)
                if mode in ["query", "album", "collection"]:
                    CU.MakeDir(self.directory + self.deviant + "/" +
                                 mode + "/" + mode_arg)
    ...
    # this implementation is old
    # it is based on the gallery being broken up into folders, each folder has 24 images
        ...
        for folder in folders:
            ...
            try:
                if mode == "favs":
                    ...
                    CU.MakeDir(self.directory + self.deviant + "/" + label)
            except OSError as mkdir_error:
                print(str(mkdir_error))
            counter = 0
            for link in pages:
    ...
    # shortcut for load_config, start, etc
    def Initialize(self, p_gallName, p_colxName):

        self.username = "Collectix"
        self.password = "dev1ant"

        # set browser so scraper can log in
        self.set_browser()
        self.login()

        artistPath = os.path.join(settings.BASE_DIR, "app_proj/media/", p_gallName)
        CU.MakeDir(artistPath)

        self.directory = artistPath

        self.deviant = p_gallName
        self.colxName = p_colxName

        CU.prog_lg.debug( self.directory )


    # this implementation is as of oct-2017
    # now must scroll the page down so deviantart loads more images with js
    # robobrowser = request + beautifulsoup
    # this capability is unavailable in beautifulsoup, but available in selenium
    def GetArtPages(self, p_artist):

        self.deviant = p_artist

        print("Getting art pages for: " + self.deviant)
        ...
        # launch selenium to gather art pages

        from selenium import webdriver
        from selenium.webdriver.common.keys import Keys

        driverPath = os.path.join(settings.BASE_DIR, "webscraper/static/geckodriver")
        botBrowser = webdriver.Firefox(executable_path=driverPath)

        botBrowser.get('https://' + self.deviant + '.deviantart.com/gallery/')

        botBrowser.maximize_window()      # make full screen so more images fit
        time.sleep(1)

        # add the pages with each page down, otherwise won't get the full gallery

        elem = botBrowser.find_element_by_tag_name("body")

        page_rx = (r"https://[a-zA-Z0-9_-]*\.deviantart\.com/art/[a-zA-Z0-9_-]*")
        pages_ls = []

        no_of_pagedowns = 10        # should give 190 images in full screen

        while no_of_pagedowns:

            html = botBrowser.page_source
            prelim = re.findall(page_rx, html, re.IGNORECASE)

            for x in prelim:
                p = str(re.sub(r'\\/', '/', x))
                if p not in pages_ls:
                    pages_ls.append(p)
    ...
    def GetArtFromPages(self, p_pages):

        # loop over the art pages, downloading the full-size art
        ...
        for link in p_pages:
            ...
            if counter % 25 == 0:
                CU.prog_lg.debug("File No: " + str(counter))
            ...
            currStatus = {}
            currStatus['file name'] = filename

            try:
                masterFileName = self.GetFileName(filename)
                currStatus['master name'] = masterFileName
            except Exception as ex:
                CU.excp_lg.error(ex)
                masterFileName = filename
                currStatus['master name'] = "* Not Parsed"

            try:
                getRes = self.get(filelink, masterFileName)
                if getRes:
                    currStatus['download'] = getRes
                else:
                    currStatus['download'] = "Downloaded"
            except Exception as ex:
                CU.excp_lg.error(ex)
                currStatus['download'] = ex
        ...
        # inputFolder = os.path.join(self.directory)
        # targetFile = inputFolder + ".zip"
        # zip_file = zipfile.ZipFile(targetFile, 'w', zipfile.ZIP_DEFLATED)

        # for root, folders, files in os.walk(inputFolder):

        #     for file_name in files:
        #         absolute_path = os.path.join(root, file_name)
        #         relative_path = os.path.join(self.deviant, file_name)
        #         zip_file.write(absolute_path, relative_path)


        results = {
            'galleryRip': status,
            #'currMedia': self.GetCurrentZips()
        }

        print("Gallery rip finished.\n")
        return results


    # helper for GetArtFromPages
    def GetFileName(self, p_filename):
        file_rx = (r"^([a-zA-Z0-9_-]+)_by_([a-zA-Z0-9_-]+)-[a-z0-9]+.([a-zA-Z]+)")
        match = re.match(file_rx, p_filename)
        ...
        if match is not None:
            ...
            newart = NS.NameSystem.StandardizeName(artname)
            newArtist = self.colxName
            fileNo = NS.NameSystem.GetFileNo(artist)
            
            newFileName = "{} ({}) {}.{}".format(newart, newArtist, fileNo, fileType)

        # the deviant art file name is gibberish

        else:
            gibberish_rx = (r"[a-zA-Z0-9_-]+.([a-zA-Z]+)")
            gmatch = re.match(gibberish_rx, p_filename)
            ...
            newFileName = "_noname ({}) {}.{}".format(newArtist, fileNo, fileType)
...
def main():
    ...
    for deviant in deviants:
        ...
        try:
            CU.MakeDir(ripper.directory + deviant)
        except OSError as mkdir_error:
            print(str(mkdir_error))
