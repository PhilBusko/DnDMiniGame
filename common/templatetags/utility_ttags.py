...
def ctrl_table(name, center=False, fullWidth=False, title=""):
    html = format_html("""
        <div class="inner_margin2" style="display: block;">
            <span style="margin-bo ttom: 4px; display: block; font-weight: bold;">{}</span>
...
@register.simple_tag
def ctrl_gallery(imgPath, title):
    html = format_html("""
        <a class="gallery_link" href="{}" > 
            <img class="gallery_thumb" src="{}" 
                alt="{}"/>
            <div class="caption_pos">
                <span class="caption_font">{}</span>
            </div>
        </a>""",
        imgPath, imgPath, title, title);
    return html
